...
Automated validation covers data shape, animation-event completeness and marker authority, append-only allocation, generated ESP round-trip, zero-area hostile-direct targeting, forbidden vanilla-asset bundling, lint, runtime gates, and SKSE compilation. All six physical marker models have been observed at ground level in game, and the bear trap's open, close, and rearm animation has been confirmed. Remaining in-game checks are blocked collision/activation, consumed/expired/save-load and cell-transition cleanup, companion/neutral immunity with the two-hostile target cap, sound direction, and high-density behavior.

자동 검증은 데이터 형식, 애니메이션 이벤트 완전성·월드 마커 권한, append-only 할당, 생성 ESP round-trip, `Area=0` 적대 대상 직접 시전, 바닐라 자산 번들 금지, lint, 런타임 gate, SKSE 컴파일을 다룹니다. 물리 마커 6종의 지면 가시성과 곰덫의 열림·닫힘·재무장 애니메이션은 인게임에서 확인했습니다. 아직 남은 인게임 검증은 충돌·활성화 차단, 발동 소모·만료·저장/로드·셀 전환 정리, 적대 대상 최대 2명과 동료·중립 무영향, 사운드 방향, 고밀도 동작입니다.
