...
**아이템 인스턴스(ExtraUniqueID) 단위로 상태를 저장/운영**하는 방식이 기본입니다.
현재 정책은 자동 어픽스 롤링보다 **룬워드/재련(Reforge) 경로 중심**입니다.
...
- 생성기 출력:
  ...
  - 런타임 동작:
  - SKSE 플러그인은 획득 시 **룬워드 조각/재련 오브** 확률 롤만 수행하고, 자동 어픽스 부여는 하지 않습니다.
  - 어픽스 부여/재롤은 재련 경로에서 수행합니다.
  - 표시(UX):
...
참고: `loot.chancePercent`는 레거시 호환 필드로 남아 있으나, 현재 기본 정책에서는 자동 어픽스 부여 게이트로 사용되지 않습니다.
...
- `CastSpellAdaptiveElement` : 대상 저항 기반으로 Fire/Frost/Shock 주문 중 최적 원소를 선택 발동
...
- `MindOverMatter` : (플레이어) `IncomingHit`에서 받은 피해 일부를 매지카로 전환(redirect)
...
`CastSpellAdaptiveElement` 필수 필드:
- `action.spells.Fire/Frost/Shock` 중 최소 1개
- `action.mode` : `WeakestResist`(기본) 등 런타임 지원 모드
- (선택) `magnitudeScaling`, `effectiveness`, `magnitudeOverride`, `applyTo`

`MindOverMatter` 필수 필드:
- `runtime.trigger = IncomingHit` (필수)
- `action.damageToMagickaPct` : (0, 100]
- (선택) `action.maxRedirectPerHit` : 0 이상
...
```json
{
  ...
  "action": {
    "type": "MindOverMatter",
    "damageToMagickaPct": 35.0,
    "maxRedirectPerHit": 120.0
  }
}
```
