...
- **함정 지면 마커가 드디어 화면에 보입니다.** 8세션에 걸친 인게임 계측으로 확정한 원인: `BSTempEffectParticle` 경로는 정적 월드 메시(곰덫 모형)에 월드 변환을 적용하지 않고(원점 고정·수명 1초 강제), 외부 구동형 아트 오브젝트 FX(소울트랩 등)는 부착돼도 재생되지 않습니다. 스스로 루프 재생하는 룬 글리프/하자드 부류만 렌더됩니다(전격 글리프 인게임 육안 확인). 이에 마커 6종을 전부 그 부류로 교체했습니다 — 곰덫→냉기 룬, 룬 함정→화염 룬, 역병 포자→독 룬(DLC02), 타르 역병→재 룬(DLC02), 흡수 포자→광란 룬(DLC02), 혼돈→전격 룬(기존 유지). 글리프 NIF는 자체 시퀀스 길이(실측 5.33초)로 수명을 덮어쓰므로, 함정 틱이 마커 만료를 감지해 단계가 남아 있으면 자동 재스폰합니다. 더 이상 참조되지 않는 커스텀 글로우 NIF와 그 패키징 테스트는 제거했습니다.

- 함정 지면 마커(곰덫 모델 등)가 로그상 스폰 수락(spawned=true)에도 화면에 전혀 렌더되지 않던 문제를 추적 중입니다. 마커·큐 스폰을 오일러 회전(NiPoint3) 오버로드에서, 실제로 렌더가 확인된 엔진 호출부들이 쓰는 항등 행렬(NiMatrix3) 오버로드로 전환했습니다(플래그 7 유지). 두 오버로드는 서로 다른 엔진 함수(29218/29219)입니다. 발밑 프로브(3모델 전부 수락·전부 미표시)로 이 가설이 기각된 뒤, 바닐라 메시 경로가 BSA에 실재함을 파서로 실측 확인하고, 검증된 렌더 사례(po3 Papyrus Extender)와의 마지막 차이인 **스폰 앵커 노드(a_target)**를 실전 마커·큐 스폰에 적용했습니다(플레이어 3인칭 3D 루트, 로그에 anchored 필드 추가). Spawn 반환값은 비동기 모델 클론 성공을 보증하지 않는다는 점(BGSParticleObjectCloneTask)도 확인했습니다.

### Added

- 함정 마커 렌더 진단용 "발밑 함정 마커 프로브" 디버그 버튼을 추가했습니다(디버그 토글이 켜진 경우에만 표시). 플레이어 발밑에 10초간 마커를 강제 스폰해, 게임플레이 변수(대상 위치·장전 타이밍·TTL)를 배제하고 "이 경로가 그려지긴 하는가"를 한 번에 판정합니다. 2차 개정에서 5변형 매트릭스(곰덫·소울트랩 각 {앵커 없음/플레이어 3D 앵커} + po3 호출 완전 복제형)로 확장했으나 전 변형이 수락-미표시로 판명, 3차 개정에서 자가진단형으로 전환했습니다: 스폰한 파티클을 보관하고 나중 시점에 비동기 모델 클론 완료 여부·씬 그래프 부모·컬링 플래그·실제 월드 변환을 로그로 역보고(`probe inspect`)하며, 디스크 exe가 SteamStub 암호화라 확인 불가였던 Spawn 함수의 런타임 해석 주소(RVA)도 함께 기록합니다(실측 결과 주소 라이브러리와 정확히 일치 — 주소 이론 종결). 4차 개정에서 검사 타이밍을 SKSE 태스크 체인(큐가 한 번에 드레인되어 전 체크포인트가 스폰 프레임에 몰리는 결함 실측)에서 실시간 타이머 스레드(0.5/2/5/9초)로 교체하고, cloneTask 생성 여부 관측과 MuImpactFramework식 `ProcessLists::globalTempEffects` 수동 등록 변형 2종을 추가했습니다. 그 실측으로 실패 기전이 확정됐습니다: 정적 월드 메시(곰덫)는 클론돼도 월드 변환이 적용되지 않고(원점 고정, lifetime 1초 강제) 아트 오브젝트류 FX(소울트랩)는 건강하게 부착되어도 시퀀스를 매직 시스템이 구동해줘야 해서 보이지 않습니다. 5차 개정은 그 반대 부류 — 스스로 루프 재생하는 룬 프로젝타일 글리프 3종(화염/냉기/전격)과 지면 하자드 NIF — 를 검사합니다. 등록 변형은 가시성과 무관하고 이중 갱신만 유발해 제거했습니다.
