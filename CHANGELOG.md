...
v1.7.3은 CTD 2건과 어픽스 발동이 영구히 멈추는 결함 1건을 수정하는 안정성 릴리스입니다. rc3는 rc2의 Prisma 패널 분할 회귀와 릴리스 검증 공백을 추가로 수정합니다. v1.7.2는 태그되지 않았으므로 이 릴리스에는 v1.7.1 이후의 모든 변경이 포함됩니다.
...
- 코세이브 직렬화 형식, MCM 옵션, ESP 레코드, 공개 효과 계약은 변경하지 않았으며 새 게임은 필요하지 않습니다.
- 어픽스 데이터와 드랍 확률은 변경하지 않았습니다.
- 게임플레이 수정 3건은 정적 분석과 회귀 테스트로 검증했으며, 실제 크래시 재현 부재는 인게임 확인이 필요합니다. Prisma 패널 수정은 실제 Chrome 로드까지 확인했으며 Skyrim/PrismaUI 인게임 확인은 남아 있습니다.
