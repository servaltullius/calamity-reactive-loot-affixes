...
- 함정 마커 렌더 진단용 "발밑 함정 마커 프로브" 디버그 버튼을 추가했습니다(디버그 토글이 켜진 경우에만 표시). 플레이어 발밑에 10초간 마커를 강제 스폰해, 게임플레이 변수(대상 위치·장전 타이밍·TTL)를 배제하고 "이 경로가 그려지긴 하는가"를 한 번에 판정합니다. 2차 개정에서 5변형 매트릭스(곰덫·소울트랩 각 {앵커 없음/플레이어 3D 앵커} + po3 호출 완전 복제형)로 확장해, 어느 열이 그려지는지로 누락 요소를 특정합니다. 결과는 로그(`trap marker probe`)와 qa_skse_logcheck 관측 그룹으로 확인합니다.
