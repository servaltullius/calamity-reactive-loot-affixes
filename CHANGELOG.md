...
- Prisma 룬워드 레시피 탐색기에 현재 인벤토리의 룬 조각 보유량을 기준으로 한 `룬 조각 준비`·`1개 부족` 필터와 레시피별 보유 진행 표시를 추가했습니다. 같은 룬이 반복되는 요구량도 합산하며, 베이스 유형·검색 조건과 함께 적용됩니다. 이 표시는 전체 레시피에 필요한 현재 보유 조각만 비교하는 읽기 전용 정보로, 기존 삽입 진행·베이스 호환성·실제 변환 가능 여부를 뜻하지 않고 저장 형식이나 드롭 밸런스를 변경하지 않습니다.
- Prisma의 `아이템 어픽스` 탭에 현재 장착 빌드 요약을 추가했습니다. 장착 효과를 공격 발동·피격/생존·처치·패시브 네 그룹으로 나누고, 현재 보정이 반영된 조건부 발동 판정률과 별도 Lucky Hit 관문, 장착 개수, 룬워드/접두/접미 구분을 표시합니다. 동일 발동 어픽스는 장착 개수와 관계없이 한 번의 판정을 공유하고, 단계형 접미는 최고 티어만 적용되며 하위 티어는 억제 상태로 표시합니다. 이 화면은 읽기 전용이며 전투 계산·저장 형식·밸런스를 변경하지 않습니다.
- 재련할 때 일반 어픽스 하나를 보호하는 선택지를 추가했습니다. 일반 재련은 기존처럼 재련 오브 1개를 사용하고, 어픽스 보호 재련은 오브 2개를 사용해 선택한 접두 또는 접미를 유지한 채 나머지만 다시 굴립니다. 완성 룬워드와 일반 어픽스 개수, 보호한 어픽스의 진화/모드 상태는 유지됩니다. UI 명령은 선택 베이스와 어픽스 토큰을 함께 보내며 서버가 현재 장비 상태를 다시 검증하므로, 장비 전환 직후의 오래된 선택은 비용·RNG 상태를 건드리기 전에 거부됩니다.

### Fixed

- 이미 같은 룬워드가 완성된 장비에 동일 레시피를 다시 변환하면 룬 조각만 재소모될 수 있던 문제를 수정했습니다. 동일 결과 룬워드는 조각 계산·소비 전에 서버에서 차단하며, 패널도 해당 레시피를 `완료` 상태로 표시합니다. 다른 룬워드를 명시적으로 선택하는 기존 교체·롤백 흐름은 유지합니다.
- 활·석궁 적중의 `HitData.weapon`이 비어 있으면 치명 시전 효과가 발동하지 않던 문제를 수정했습니다. 실제 투사체의 활·석궁 `weaponSource` 또는 현재 원거리 공격 데이터가 확인될 때만 보완 판정을 허용하고, 주문 적중과 단순 장착 무기 fallback은 계속 차단해 발동 주문의 재귀 시전을 막습니다.
- 플레이어 소환물의 주문 적중이 플레이어가 장착한 무기 때문에 함정 생성 조건의 `weapon hit`으로 오인될 수 있던 문제를 수정했습니다. 함정 판정은 실제 적중 공격자를 우선 사용하고, 주문 적중은 장착 무기로 승격하지 않습니다. 실제 플레이어 근접·활·석궁 적중은 그대로 허용합니다.
- 재련할 때 기존 2·3옵션 장비도 신규 전리품의 70/22/8% 옵션 수 분포를 다시 굴려 1옵션으로 줄어들 수 있던 문제를 수정했습니다. 이제 재련은 현재 일반 어픽스 개수를 유지한 채 같은 개수만 다시 굴리며, 일반 어픽스가 없는 베이스만 최소 1개를 얻습니다. 완성 룬워드는 기존처럼 별도 보존됩니다.

## [1.7.5] - 2026-08-11

v1.7.5는 함정 6종의 지속 마커를 바닐라 물리 모델로 전환하고, 곰덫의 실제 열림·발동·재장전 애니메이션을 복원한 함정 시인성 릴리스입니다. 곰덫 외 범위 함정은 엔진 광역 적용 대신 런타임이 적대 대상만 골라 최대 2명에게 효과를 적용하며, 시체 폭발 대상 수명과 코세이브 writer·장착 효과 재계산 회귀 안전망도 보강했습니다. rc1은 바닐라 모델 6종의 지면 표시와 곰덫 애니메이션을 인게임에서 확인한 첫 공개 테스트 빌드입니다. rc2는 진단 전용 파티클을 실제 함정과 같은 월드 레퍼런스 생성·애니메이션·정리 경로로 교체하고, 6종 표시와 자동 정리를 인게임에서 다시 확인한 QA·릴리스 강화 빌드입니다.
...
- rc2는 rc1의 ESP FormID, 코세이브 wire format, 게임플레이 수치와 드랍 확률을 변경하지 않습니다. 기존 세이브를 그대로 사용할 수 있으며 새 게임은 필요하지 않습니다.
