...
- 함정 마커 렌더 진단용 "발밑 함정 마커 프로브" 디버그 버튼을 추가했습니다(디버그 토글이 켜진 경우에만 표시). 플레이어 발밑에 10초간 마커를 강제 스폰해, 게임플레이 변수(대상 위치·장전 타이밍·TTL)를 배제하고 "이 경로가 그려지긴 하는가"를 한 번에 판정합니다. 2차 개정에서 5변형 매트릭스(곰덫·소울트랩 각 {앵커 없음/플레이어 3D 앵커} + po3 호출 완전 복제형)로 확장했으나 전 변형이 수락-미표시로 판명, 3차 개정에서 자가진단형으로 전환했습니다: 스폰한 파티클을 보관하고 0.5/2/5초 시점에 비동기 모델 클론 완료 여부·씬 그래프 부모·컬링 플래그·실제 월드 변환을 로그로 역보고(`probe inspect`)하며, 디스크 exe가 SteamStub 암호화라 확인 불가였던 Spawn 함수의 런타임 해석 주소(RVA)도 함께 기록합니다. 실패 단계를 눈이 아니라 로그로 특정하기 위한 것입니다.
