...
- 함정 지면 마커(곰덫 모델 등)가 로그상 스폰 수락(spawned=true)에도 화면에 전혀 렌더되지 않던 문제를 추적 중입니다. 마커·큐 스폰을 오일러 회전(NiPoint3) 오버로드에서, 실제로 렌더가 확인된 엔진 호출부들이 쓰는 항등 행렬(NiMatrix3) 오버로드로 전환했습니다(플래그 7 유지). 두 오버로드는 서로 다른 엔진 함수(29218/29219)입니다.

### Added

- 함정 마커 렌더 진단용 "발밑 함정 마커 프로브" 디버그 버튼을 추가했습니다(디버그 토글이 켜진 경우에만 표시). 곰덫·소울트랩·커스텀 글로우 마커 3종을 플레이어 발밑에 10초간 강제 스폰해, 게임플레이 변수(대상 위치·장전 타이밍·TTL)를 배제하고 "이 경로가 그려지긴 하는가"를 한 번에 판정합니다. 결과는 로그(`trap marker probe`)와 qa_skse_logcheck 관측 그룹으로 확인합니다.
