...
- (프로토타입) 화염 전환(Fire Infusion)의 대미지 효과에 적용 시 히트 사운드(SNDD On Hit, 바닐라 `MAGFireboltImpactSD`)를 추가했습니다. 바닐라가 시전 사이클 없는 적용(물약·의복 인챈트)에 쓰는 슬롯과 같은 방식이며, 화염 1종으로 인게임 재생을 확인한 뒤 냉기·전격 전환으로 확대할 예정입니다. 냉기·전격은 의도적으로 무음 대조군으로 남겨 두었습니다.
