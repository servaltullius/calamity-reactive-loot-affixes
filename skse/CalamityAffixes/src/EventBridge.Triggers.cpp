...
namespace CalamityAffixes
{
	...
	void EventBridge::ProcessTrigger(Trigger a_trigger, RE::Actor* a_owner, RE::Actor* a_target, const RE::HitData* a_hitData)
	{
		...
		const float lowHealthCurrentPct = hasLowHealthSnapshot ? ResolveLowHealthTriggerCurrentPct(a_trigger, a_owner) : 100.0f;
		const float lowHealthPreviousPct = ResolveLowHealthTriggerPreviousPct(lowHealthOwnerFormID);
		...
		for (const auto i : *indices) {
			...
			if (!PassesTriggerProcPreconditions(
					...
					a_hitData,
					now,
					lowHealthPreviousPct,
					lowHealthCurrentPct,
					&perTargetKey)) {
				continue;
			}

			const float chance = ResolveTriggerProcChancePct(affix, i);
			...
			CommitTriggerProcRuntime(affix, perTargetKey, usesPerTargetIcd, chance, now);
		...
		if (hasLowHealthSnapshot) {
			...
			StoreLowHealthTriggerSnapshot(lowHealthOwnerFormID, lowHealthCurrentPct);
		}
