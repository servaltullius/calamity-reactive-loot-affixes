...
namespace CalamityAffixes
{
		namespace
		{
			struct LowHealthSnapshot
			{
				RE::FormID ownerFormID{ 0u };
	...
	void EventBridge::ProcessTrigger(Trigger a_trigger, RE::Actor* a_owner, RE::Actor* a_target, const RE::HitData* a_hitData)
	{
		...
		const auto buildLowHealthSnapshot = [&]() noexcept {
			LowHealthSnapshot snapshot{};
			snapshot.ownerFormID = (a_trigger == Trigger::kLowHealth && a_owner) ? a_owner->GetFormID() : 0u;
			snapshot.hasSnapshot = snapshot.ownerFormID != 0u;
			snapshot.currentPct = snapshot.hasSnapshot ? ResolveLowHealthTriggerCurrentPct(a_trigger, a_owner) : 100.0f;
			snapshot.previousPct = ResolveLowHealthTriggerPreviousPct(snapshot.ownerFormID);
			return snapshot;
		};

		const auto now = std::chrono::steady_clock::now();
		const auto lowHealthSnapshot = buildLowHealthSnapshot();
