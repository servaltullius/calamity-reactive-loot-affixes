...
- **Leaf** (`Tir,Ral`) — 적중 22% / ICD 7초 → 대상
  - Health(ResistFire) 25
...
- **Mist** (`Cham,Shael,Gul,Thul,Ith`) — 적중 18% / ICD 5초 → 대상
  - SpeedMult -30, 4초
  - 서리 파편이 적의 움직임을 무겁게 둔화
...
- **Crescent Moon** (`Shael,Um,Tir`) — 적중 24% / ICD 6초 → 대상
  - Health(ResistShock) 35
  - 번개를 실은 일격으로 적에게 추가 전격 피해
...
- **Venom** (`Tal,Dol,Mal`) — 적중 24% / ICD 6초 → 대상
  - Health(PoisonResist) 5, 6초
  - 독이 혈류를 타고 번져 지속 중독 피해
...
- **Rift** (`Hel,Ko,Lem,Gul`) — 적중 8% / ICD 18초 → 대상
  - Calm, Aggression 20, 4초
...
- **Melody** (`Shael,Ko,Nef`) — 적중 22% / ICD 8초 → 자신
  - BowSpeedBonus +25, 7초
  - 질풍의 흐름으로 활 공격 속도 상승
...
- **Steel** (`Tir,El`) — 적중 18% / ICD 8초 → 자신
  - WeaponSpeedMult +20, 5초
  - 연타 가속으로 무기 공격 속도를 끌어올림

- **Memory** (`Lum,Io,Sol,Eth`) — 적중 20% / ICD 9초 → 자신
  - StaminaRateMult +100, 8초
  - 전투 기억을 되살려 스태미나 회복 가속

- **Harmony** (`Tir,Ith,Sol,Ko`) — 적중 26% / ICD 8초 → 자신
  - SpeedMult +25, 5초
  - 가속으로 빈틈을 메워 기동 우위 확보

- **Passion** (`Dol,Ort,Eld,Lem`) — 적중 26% / ICD 7초 → 자신
  - AttackDamageMult +18, 6초
  - 가속으로 빈틈을 메워 기동 우위 확보
...
- **Hearth** (`Shael,Io,Thul`) — 피격 22% / ICD 11초 → 자신
  - ResistFrost +40, 7초
  - 화덕의 온기로 냉기 저항을 올려 방어

- **Temper** (`Shael,Io,Ral`) — 피격 18% / ICD 8초 → 자신
  - AttackDamageMult +15, 8초
  - 피격의 열기를 공격력으로 전환해 반격 강화

- **Splendor** (`Eth,Lum`) — 피격 18% / ICD 10초 → 자신
  - ResistMagic +30, 7초
  - 마력 방패로 마법 저항 전반을 강화

- **Duress** (`Shael,Um,Thul`) — 피격 24% / ICD 10초 → 자신
  - DamageResist +100, 7초
  - 수호막으로 방어도를 일시 대폭 상승

- **Stone** (`Shael,Um,Pul,Lum`) — 피격 24% / ICD 11초 → 자신
  - DamageResist +120, 8초
  - 석화로 피부를 돌처럼 굳혀 강력 방어
...
- **Wisdom** (`Pul,Ith,Eld`) — 피격 18% / ICD 11초 → 자신
  - HealRate +5, 7초
  - 지혜의 파동으로 상처를 서서히 치유
...
- **Gloom** (`Fal,Um,Pul`) — 피격 7% / ICD 24초 → 자신
  - Invisibility, Invisibility, 3초
  - 투명화로 전투를 끊고 각 재설정
