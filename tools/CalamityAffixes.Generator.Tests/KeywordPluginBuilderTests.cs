...
public sealed class KeywordPluginBuilderTests
{
    ...
    [Fact]
    public void BuildKeywordPlugin_WhenHybridDropConfigured_DoesNotInjectTargetLeveledLists()
    {
        var spec = new AffixSpec
        {
            Version = 1,
        ...
        Assert.DoesNotContain(mod.LeveledItems, list => list.FormKey == new FormKey(ModKey.FromNameAndExtension("Skyrim.esm"), 0x00068525));
        Assert.DoesNotContain(mod.LeveledItems, list => list.FormKey == new FormKey(ModKey.FromNameAndExtension("Skyrim.esm"), 0x000C3C9B));
    }
